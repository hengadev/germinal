			...
			<tbody class="divide-y divide-border-card">
				{#each filteredReservations() as reservation}
					...
					<tr class="hover:bg-dark-50 transition-colors">
						...
						<td class="px-6 py-4">
							<div class="text-sm">
								<div class="font-medium text-dark-900">{reservation.sessionTitle}</div>
		...
		{#each filteredReservations() as reservation}
			...
			<div class="bg-white rounded-lg border border-border-card p-4">
				...
				<div class="grid grid-cols-2 gap-3 mb-3">
					<div>
						<div class="text-xs text-dark-400">Session</div>
						<div class="text-sm font-medium text-dark-900 truncate">{reservation.sessionTitle}</div>
					</div>
					<div>
