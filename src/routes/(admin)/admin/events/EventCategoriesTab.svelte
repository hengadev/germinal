 ...
 <input id="createCatIcon" name="icon" type="text" bind:value={createIcon} placeholder="calendar (nom d'icône Lucide)"
    ...
    {:else if fieldName === "color"}
        <div class="flex items-center gap-3">
            <input id="createCatColor" type="color" bind:value={createColor} class="w-12 h-10 border border-border-input rounded-lg cursor-pointer" />
...
{#snippet editInput(fieldName: string)}
 ...
 {:else if fieldName === "color"}
 <div class="flex items-center gap-3">
 <input id="editCatColor" type="color" bind:value={editColor} class="w-12 h-10 border border-border-input rounded-lg cursor-pointer" />
...
{#if categories.length === 0}
 ...
 <p class="text-muted-foreground mb-6">Créez votre première catégorie pour organiser vos événements</p>
 </div>
{:else}
 ...
 <th class="px-6 py-4 text-left text-xs font-semibold text-foreground-alt uppercase tracking-wider">Événements</th>
 ...
 {/if}
 <span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-muted text-foreground-alt rounded-full">
 <Hash size={12} /> {category.eventCount ?? 0}
 </span>
 <span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-muted text-foreground-alt rounded-full">
...
{#if isMobile}
 ...
 <p class="text-muted-foreground text-sm">Créer une nouvelle catégorie d'événement</p>
        </div>
 ...
 <input id="createCatPublishedM" name="published" type="checkbox" value="true" bind:checked={createPublished}
 class="w-4 h-4 text-foreground border-border-input rounded focus:ring-foreground" />
 <div>
 <label for="createCatPublishedM" class="block text-sm font-medium text-foreground cursor-pointer">Publié</label>
...
{:else}
 ...
 <div class="col-span-2">{@render field("displayNameFr", "Nom d'affichage (Français)", createInput, null)}</div>
            ...
            <div class="flex items-center gap-3 p-4 bg-muted rounded-lg">
                <input id="createCatPublishedD" name="published" type="checkbox" value="true" bind:checked={createPublished}
                    class="w-5 h-5 text-foreground border-border-input rounded focus:ring-foreground" />
                <div>
                    <label for="createCatPublishedD" class="block text-sm font-medium text-foreground cursor-pointer">Publié</label>
                    <p class="text-xs text-muted-foreground">Afficher cette catégorie sur le site web</p>
                </div>
            </div>
            <div class="flex w-full justify-end gap-3 pt-2">
...
{#if isMobile}
 ...
 <div class="flex items-center gap-3 p-3 bg-muted rounded-lg">
 <input id="editCatPublishedM" name="published" type="checkbox" value="true" bind:checked={editPublished}
 class="w-4 h-4 text-foreground border-border-input rounded focus:ring-foreground" />
 <div>
 <label for="editCatPublishedM" class="block text-sm font-medium text-foreground cursor-pointer">Publié</label>
 <p class="text-xs text-muted-foreground">Afficher cette catégorie sur le site web</p>
 </div>
 </div>
 </div>
...
{:else}
 ...
 <div class="flex items-center gap-3 p-4 bg-muted rounded-lg">
 <input id="editCatPublishedD" name="published" type="checkbox" value="true" bind:checked={editPublished}
 class="w-5 h-5 text-foreground border-border-input rounded focus:ring-foreground" />
 <div>
 <label for="editCatPublishedD" class="block text-sm font-medium text-foreground cursor-pointer">Publié</label>
 <p class="text-xs text-muted-foreground">Afficher cette catégorie sur le site web</p>
 </div>
 </div>
 <div class="flex w-full justify-end gap-3 pt-2">
