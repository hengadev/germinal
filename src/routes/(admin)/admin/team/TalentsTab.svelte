<script lang="ts">
    ...
    let { talents, categories, form }: Props = $props();
...
{#if activeTab === 'talents'}
    {#if talents.length === 0}
        <div class="bg-background rounded-lg border border-border-card p-12 text-center">
            ...
            <h3 class="text-xl font-semibold text-foreground mb-2">Aucun talent pour le moment</h3>
            <p class="text-muted-foreground mb-6">Ajoutez votre premier talent pour commencer</p>
            <a
                href="/admin/talents/new"
                class="inline-flex items-center gap-2 px-4 py-2 bg-foreground text-background rounded-lg hover:opacity-90 transition-colors"
            >
        ...
        <div class="bg-background rounded-lg border border-border-card overflow-hidden hidden lg:block">
            <table class="w-full">
                <thead class="bg-muted border-b border-border-card">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-foreground-alt uppercase tracking-wider">Talent</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-foreground-alt uppercase tracking-wider">Rôle</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-foreground-alt uppercase tracking-wider">Bio</th>
                ...
                <tbody class="divide-y divide-border-card">
                    {#each talents as talent (talent.id)}
                        <tr class="hover:bg-muted transition-colors cursor-pointer" onclick={() => goto(`/admin/talents/${talent.id}`)}>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    {#if talent.profileMedia?.url}
                                        <img src={talent.profileMedia.url} alt="{talent.firstName} {talent.lastName}" class="w-14 h-14 object-cover rounded-full" />
                                    {:else}
                                        <div class="w-14 h-14 bg-muted rounded-full flex items-center justify-center">
                                            <User size={24} class="text-muted-foreground" />
                                        </div>
                                    {/if}
                                    <div class="font-medium text-foreground">{talent.firstName} {talent.lastName}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2 text-sm text-foreground-alt">
                                    <Briefcase size={16} />
                                    <span class="truncate max-w-[200px]">{talent.roleEn}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm text-muted-foreground line-clamp-2 max-w-[300px]">{talent.bioEn}</p>
                            </td>
                            <td class="px-6 py-4">
                                {#if talent.published}
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-foreground text-background rounded-full">
                                        <Eye size={14} /> Publié
                                    </span>
                                {:else}
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button
                                        onclick={(e) => { e.stopPropagation(); openDeleteDialog(talent); }}
            ...
            {#each talents as talent (talent.id)}
                ...
                >
                    <div class="flex gap-4 mb-4">
                        {#if talent.profileMedia?.url}
                            <img src={talent.profileMedia.url} alt="{talent.firstName} {talent.lastName}" class="w-16 h-16 object-cover rounded-full flex-shrink-0" />
                        {:else}
                            <div class="w-16 h-16 bg-muted rounded-full flex items-center justify-center flex-shrink-0">
                                <User size={32} class="text-muted-foreground" />
                            </div>
                        {/if}
                        <div class="min-w-0 flex-1">
                            <h3 class="font-semibold text-foreground truncate">{talent.firstName} {talent.lastName}</h3>
                            <div class="flex items-center gap-1 text-sm text-foreground-alt mt-1">
                                <Briefcase size={14} />
                                <span class="truncate">{talent.roleEn}</span>
                            </div>
                            <p class="text-sm text-muted-foreground line-clamp-2 mt-1">{talent.bioEn}</p>
                        </div>
                    </div>
    ...
    {/if}
{:else}
    <TalentCategoriesTab {categories} {isMobile} bind:createDialogOpen={catCreateDialogOpen} />
{/if}
    ...
    <Drawer bind:isOpen={deleteDialogOpen}>
        <div class="sticky top-0 bg-background pb-4 border-b border-border-card -mx-4 px-4 -mt-4 pt-4 z-10">
            <div class="flex items-center justify-between mb-2">
                <h2 class="text-xl font-semibold tracking-tight">Supprimer le Talent</h2>
                <button type="button" onclick={() => (deleteDialogOpen = false)} class="p-2 hover:bg-muted rounded-md transition-colors">
                    <X class="text-foreground size-5" />
                </button>
            </div>
            <p class="text-muted-foreground text-sm">
                Êtes-vous sûr de vouloir supprimer "{selectedTalent?.firstName} {selectedTalent?.lastName}" ? Cette action ne peut pas être annulée.
            </p>
        </div>
        <div class="pt-4">
