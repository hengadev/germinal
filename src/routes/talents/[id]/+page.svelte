<script lang="ts">
  ...
  let socialLinks = $derived(
    data.talent.socialLinks
      ? JSON.parse(data.talent.socialLinks)
      : {}
  );
</script>
...
<article class="container mx-auto px-4 py-12 max-w-4xl">
  <div class="flex flex-col md:flex-row gap-8 mb-8">
    ...
    {:else}
      <div class="w-48 h-48 flex-shrink-0 bg-gray-200 rounded-lg flex items-center justify-center">
    ...
    <div class="flex-1">
      <h1 class="text-5xl font-bold mb-2">
        {data.talent.firstName} {data.talent.lastName}
      </h1>
      <p class="text-xl text-gray-600 mb-4">{data.talent.role}</p>
      <p class="text-lg text-gray-700 leading-relaxed">{data.talent.bio}</p>
